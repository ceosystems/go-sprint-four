...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	k1 := (runningCaloriesMeanSpeedMultiplier * avgSpeed * runningCaloriesMeanSpeedShift) * weight
	return (k1 / mInKm * duration * minInH)
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	metabolism := walkingCaloriesWeightMultiplier * weight                            // 0.035 * ВесСпортсменаВКг
	speed := meanSpeed(action, duration) * kmhInMsec                                  //СредняяСкоростьВМетрахВСекунду
	speedPow := math.Pow(speed, 2)                                                    //(СредняяСкоростьВМетрахВСекунду**2)
	Sum := metabolism + (speedPow/(height/cmInM))*walkingSpeedHeightMultiplier*weight // ((0.035 * ВесСпортсменаВКг + (СредняяСкоростьВМетрахВСекунду**2 / РостВМетрах) * 0.029 * ВесСпортсменаВКг)
	caloriesBurned := Sum * duration * minInH                                         //* ВремяТренировкиВЧасах * minInH)
